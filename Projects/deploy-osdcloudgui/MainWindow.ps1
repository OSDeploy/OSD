...
if ($CustomImageChildItem) {
    $OSCatalogAmd64 = Get-OSDCloudOperatingSystems -OSArch amd64
...
$formMainWindowControlStartButton.add_Click({
    ...
    if ($formMainWindowControlOSNameCombobox.SelectedValue -like 'Windows 1*') {
        ...
        if ($Global:OSDCloudGUI.OSArchitecture -match 'arm64') {
            ...
            $OSDCloudOperatingSystem = Get-OSDCloudOperatingSystems -OSArch amd64 | Where-Object {$_.Name -match $OSNameMatch} | Where-Object {$_.Activation -eq $OSActivation} | Where-Object {$_.Language -eq $OSLanguage}
        }
