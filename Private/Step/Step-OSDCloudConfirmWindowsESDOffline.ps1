function Step-OSDCloudConfirmWindowsESDOffline {
    ...
    .EXAMPLE
    Step-OSDCloudConfirmWindowsESDOffline -OperatingSystemObject $OS -DownloadPath 'D:\OSDCloud\OS'
    ...
    Write-Host -ForegroundColor DarkGray "[$(Get-Date -format s)] Confirm WindowsESD Offline Availability"
    #=================================================
    ...
    if (-not $CacheWindowsESD) {
        Write-Host -ForegroundColor DarkGray "[$(Get-Date -format s)] OperatingSystem is not in the OSDCoreCache"
        return
    }
    #=================================================
    ...
    if ($OperatingSystemObject.SHA1) {
        ...
        if ($SourceFileHash.Hash -ne $OperatingSystemObject.SHA1) {
            ...
            Write-Host -ForegroundColor DarkYellow "[$(Get-Date -format s)] [$($MyInvocation.MyCommand.Name)] SHA1 hash mismatch for cached source file: $($CacheWindowsESD.FullName)"
            return
        }
    }
    if ($OperatingSystemObject.SHA256) {
        ...
        if ($SourceFileHash.Hash -ne $OperatingSystemObject.SHA256) {
            # Hash mismatch means the source cannot be trusted; skip copy.
            Write-Host -ForegroundColor DarkYellow "[$(Get-Date -format s)] [$($MyInvocation.MyCommand.Name)] SHA256 hash mismatch for cached source file: $($CacheWindowsESD.FullName)"
            return
        }
    }
    Write-Host -ForegroundColor DarkGray "[$(Get-Date -format s)] OperatingSystemObject is available in the OSDCoreCache. OK."
    $global:OSDCloudDeploy.ConfirmWindowsESDOffline = $true
    #=================================================
