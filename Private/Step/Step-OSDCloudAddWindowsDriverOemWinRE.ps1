function Step-OSDCloudAddWindowsDriverOemWinRE {
    ...
    if ($env:SystemDrive -ne 'X:') {
        Write-Host -ForegroundColor DarkGray "[$(Get-Date -format s)] [$($MyInvocation.MyCommand.Name)] This step should only run in WinPE (X:). Skipping WinRE driver injection."
        return
    }
    #=================================================
