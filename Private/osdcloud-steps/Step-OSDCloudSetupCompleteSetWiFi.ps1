function Step-OSDCloudSetupCompleteSetWiFi {
    [CmdletBinding()]
    param ()
    Write-Host -ForegroundColor DarkGray "[$(Get-Date -format s)] [$($MyInvocation.MyCommand.Name)] Adding WinPE WiFi Profile to SetupComplete.cmd"
    ...
    if (Test-Path $WifiJson) {
        ...
        if (Test-Path -Path $SetupCompletePs){
            Write-Host -ForegroundColor DarkGray "[$(Get-Date -format s)] [$($MyInvocation.MyCommand.Name)] Set-WiFi -SSID `"$SSID`" -PSK `"***********`""
            Add-Content -Path $SetupCompletePs ' Write-Host "[$(Get-Date -format s)] [$($MyInvocation.MyCommand.Name)] Creating WiFi Profile"'
