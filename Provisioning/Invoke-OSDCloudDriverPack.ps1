...
if (Test-Path 'C:\Drivers') {
    ...
    foreach ($Item in $DriverPacks) {
        ...
        Write-Verbose -Verbose "[$(Get-Date -format s)] Reviewing $ExpandFile"
        $DestinationPath = Join-Path $Item.Directory $Item.BaseName
        #=================================================
        #   Zip
        #=================================================
        if ($Item.Extension -eq '.zip') {
            Write-Verbose -Verbose "[$(Get-Date -format s)] Processing actions for ZIP file"

            if (Test-Path "$DestinationPath") {
                Write-Verbose -Verbose "[$(Get-Date -format s)] DriverPack has already been extracted"
            }
            else {
                Write-Verbose -Verbose "[$(Get-Date -format s)] Expanding ZIP DriverPack to $DestinationPath"
                Expand-Archive -Path $ExpandFile -DestinationPath $DestinationPath -Force

                Write-Verbose -Verbose "[$(Get-Date -format s)] Applying DriverPack with PNPUNATTEND"
        ...
        if ($Item.Extension -eq '.cab') {
            Write-Verbose -Verbose "[$(Get-Date -format s)] Processing actions for CAB file"
            ...
            if (Test-Path "$DestinationPath") {
                ...
                Write-Verbose -Verbose "[$(Get-Date -format s)] Expanding CAB DriverPack to $DestinationPath"
        ...
        if ($Item.Extension -eq '.exe') {
            if ($Item.VersionInfo.FileDescription -match 'Dell') {
                Write-Verbose -Verbose "[$(Get-Date -format s)] Processing actions for Dell EXE file"
                Write-Verbose -Verbose "[$(Get-Date -format s)] FileDescription: $($Item.VersionInfo.FileDescription)"
                Write-Verbose -Verbose "[$(Get-Date -format s)] ProductVersion: $($Item.VersionInfo.ProductVersion)"

                if (Test-Path "$DestinationPath") {
                    Write-Verbose -Verbose "[$(Get-Date -format s)] DriverPack has already been extracted"
                }
                else {
                    Write-Verbose -Verbose "[$(Get-Date -format s)] Expanding Dell DriverPack to $DestinationPath"
        ...
        if ($Item.Extension -eq '.exe') {
            if (($Item.VersionInfo.InternalName -match 'hpsoftpaqwrapper') -or ($Item.VersionInfo.OriginalFilename -match 'hpsoftpaqwrapper.exe') -or ($Item.VersionInfo.FileDescription -like "HP *")) {
                Write-Verbose -Verbose "[$(Get-Date -format s)] Processing actions for HP EXE file"
                Write-Verbose -Verbose "[$(Get-Date -format s)] FileDescription: $($Item.VersionInfo.FileDescription)"
                Write-Verbose -Verbose "[$(Get-Date -format s)] ProductVersion: $($Item.VersionInfo.ProductVersion)"
                Write-Verbose -Verbose "[$(Get-Date -format s)] InternalName: $($Item.VersionInfo.InternalName)"
                Write-Verbose -Verbose "[$(Get-Date -format s)] OriginalFilename: $($Item.VersionInfo.OriginalFilename)"

                if (Test-Path "$DestinationPath") {
                    Write-Verbose -Verbose "[$(Get-Date -format s)] DriverPack has already been extracted"
                }
                else {
                    Write-Verbose -Verbose "[$(Get-Date -format s)] Expanding HP DriverPack to $DestinationPath"
        ...
        if ($Item.Extension -eq '.exe') {
            if (($Item.VersionInfo.FileDescription -match 'Lenovo') -or ($Item.Name -match 'tc_') -or ($Item.Name -match 'tp_') -or ($Item.Name -match 'ts_') -or ($Item.Name -match '500w') -or ($Item.Name -match 'sccm_') -or ($Item.Name -match 'm710e') -or ($Item.Name -match 'tp10') -or ($Item.Name -match 'tp8') -or ($Item.Name -match 'yoga')) {
                Write-Verbose -Verbose "[$(Get-Date -format s)] Processing actions for Lenovo EXE file"
                Write-Verbose -Verbose "[$(Get-Date -format s)] FileDescription: $($Item.VersionInfo.FileDescription)"
                Write-Verbose -Verbose "[$(Get-Date -format s)] ProductVersion: $($Item.VersionInfo.ProductVersion)"
                ...
                if (Test-Path "$DestinationPath") {
                    ...
                    Write-Verbose -Verbose "[$(Get-Date -format s)] Expanding Lenovo DriverPack to $DestinationPath"
        ...
        if ($Item.Extension -eq '.msi') {
            Write-Verbose -Verbose "[$(Get-Date -format s)] Processing actions for MSI file"
        ...
        #   Everything Else
        #=================================================
        Write-Verbose -Verbose "[$(Get-Date -format s)] File does not appear to be a DriverPack"
        #=================================================
    }
}
#=================================================
