...
if ($WindowsPhase -eq 'Windows') {
    ...
    if ($fromIsoUrl) {
        ...
        $InstallModule = $false
        $PSModuleName = 'OSD'
        Write-Host -ForegroundColor DarkGray "[$(Get-Date -format s)] Test $PSModuleName Module"
        ...
        Write-Host -ForegroundColor DarkGray "[$(Get-Date -format s)] Downloading $fromIsoUrl"
        ...
        if (-NOT (Test-Path $OSDCloudRERoot)) {
           Write-Error "[$(Get-Date -format s)] Unable to find OSDCloudRE Root at $OSDCloudRERoot"
            Break
        }
        #============================================
        #	Update WinPE Volume
        ...
        if ((Test-Path -Path "$OSDCloudREMedia") -and (Test-Path -Path "$OSDCloudRERoot")) {
            Write-Host -ForegroundColor DarkGray "[$(Get-Date -format s)] Copying $OSDCloudREMedia to OSDCloud WinPE partition at $OSDCloudRERoot"
        ...
        #============================================
        Write-Host -ForegroundColor DarkGray "[$(Get-Date -format s)] Removing Read Only attributes in $OSDCloudRERoot"
        ...
        if ($MountDiskImage) {
            Start-Sleep -Seconds 3
            Write-Host -ForegroundColor DarkGray "[$(Get-Date -format s)] Dismounting ISO at $($MountDiskImage.ImagePath)"
