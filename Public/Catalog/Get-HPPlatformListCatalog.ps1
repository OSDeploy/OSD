...
.EXAMPLE
$Results = Get-HPPlatformListCatalog
...
function Get-HPPlatformListCatalog {
    ...
    $BuildCatalogFile		= Join-Path $env:TEMP (Join-Path 'OSD' 'HPPlatformListCatalog.xml')
    $OfflineCatalogFile     = "$($MyInvocation.MyCommand.Module.ModuleBase)\Catalogs\HPPlatformListCatalog.xml"
