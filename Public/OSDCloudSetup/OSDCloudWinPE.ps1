function Edit-OSDCloudWinPE {
    ...
    if ($PSBoundParameters.ContainsKey('WorkspacePath')) {
        Write-Host -ForegroundColor DarkGray "[$(Get-Date -format s)] Setting Workspace Path"
    ...
    #=================================================
    Write-Host -ForegroundColor DarkGray "[$(Get-Date -format s)] Mounting $WorkspacePath\Media\Sources\boot.wim"
    ...
    if (Test-Path "$WorkspacePath\Config") {
        Write-Host -ForegroundColor DarkGray "[$(Get-Date -format s)] Copying $WorkspacePath\Config to X:\OSDCloud\Config"
    ...
    if (Test-Path "$WorkspacePath\ODT") {
        Write-Host -ForegroundColor DarkGray "[$(Get-Date -format s)] Copying $WorkspacePath\ODT to X:\OSDCloud\ODT"
    ...
    #region WifiProfile
    if ($WifiProfile) {
        Write-Host -ForegroundColor DarkGray "[$(Get-Date -format s)] Adding WiFi Profile $WifiProfile"
...
"@
    Write-Host -ForegroundColor DarkGray "[$(Get-Date -format s)] Startnet.cmd: Reset to defaults"
    $StartnetCMD
    ...
    if ($StartPSCommand) {
        ...
        Write-Host -ForegroundColor DarkGray "[$(Get-Date -format s)] Startnet.cmd: start /wait PowerShell -NoL -C `"$StartPSCommand`""
    ...
    if ($StartOSDCloud) {
        Write-Host -ForegroundColor DarkGray "[$(Get-Date -format s)] Startnet.cmd: Launch Start-OSDCloud on WinPE Startup"
    ...
    if ($StartOSDCloudGUI) {
        Write-Host -ForegroundColor DarkGray "[$(Get-Date -format s)] Startnet.cmd: Launch Start-OSDCloudGUI on WinPE Startup"
    ...
    if ($StartOSDCloud -or $StartOSDCloudGUI -or $StartURL -or $StartOSDPad -or $Startnet) {
        ...
        Write-Host -ForegroundColor DarkGray "[$(Get-Date -format s)] Startnet.cmd: Start PowerShell in new Window"
        Write-Host '@ECHO OFF'
        Add-Content -Path "$MountPath\Windows\System32\Startnet.cmd" -Value '@ECHO OFF' -Force
    ...
    if ($Wallpaper) {
        Write-Host -ForegroundColor DarkGray "[$(Get-Date -format s)] Adding Wallpaper $Wallpaper"
    ...
    if (Test-Path "$WorkspacePath\cmtrace.exe") {
        Write-Host -ForegroundColor DarkGray "[$(Get-Date -format s)] Copy CMTrace from WorkSpace to WinPE"
    ...
    Write-Host -ForegroundColor DarkGray "[$(Get-Date -format s)] Saving OSD Module to X:\Program Files\WindowsPowerShell\Modules"
    ...
    if ($AddAzure.IsPresent) {
        Write-Host -ForegroundColor DarkGray "[$(Get-Date -format s)] Saving Azure Modules to X:\Program Files\WindowsPowerShell\Modules"
        ...
        Write-Host -ForegroundColor DarkGray "[$(Get-Date -format s)] Saving AzCopy to X:\Windows"
    ...
    if ($AddAws.IsPresent) {
        Write-Host -ForegroundColor DarkGray "[$(Get-Date -format s)] Saving AWS Modules to X:\Program Files\WindowsPowerShell\Modules"
    ...
    Write-Host -ForegroundColor Cyan "[$(Get-Date -format s)] Create OSDCloud Workspace ISOs"
    New-OSDCloudISO
    #endregion
