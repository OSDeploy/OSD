...
function Update-OSDCloudUSB {
    ...
    if ($UsbVolumes) {
        Write-Host -ForegroundColor DarkGray "$((Get-Date).ToString('OSDy-MM-dd-HHmmss')) USB volumes found"
        Write-Host -ForegroundColor DarkGray "========================================================================="
    }
    else {
        Write-Warning "$((Get-Date).ToString('OSDy-MM-dd-HHmmss')) Unable to find any USB volumes"
        Write-Warning "$((Get-Date).ToString('OSDy-MM-dd-HHmmss')) Plug in a USB drive first"
        Write-Host -ForegroundColor DarkGray "========================================================================="
        Break
    }
    #=================================================
    ...
    #=================================================
    if (! $WorkspacePath) {
        Write-Warning "$((Get-Date).ToString('OSDy-MM-dd-HHmmss')) OSDCloud Workspace is not present on this system"
        Write-Warning "$((Get-Date).ToString('OSDy-MM-dd-HHmmss')) You will not be able to update the WinPE volume"
        Write-Host -ForegroundColor DarkGray "========================================================================="
        $RobocopyWorkspace = $false
    }
    elseif (! (Test-Path $WorkspacePath)) {
        Write-Warning "$((Get-Date).ToString('OSDy-MM-dd-HHmmss')) OSDCloud Workspace is not at the path $WorkspacePath"
        Write-Warning "$((Get-Date).ToString('OSDy-MM-dd-HHmmss')) You will not be able to update the WinPE volume"
        Write-Host -ForegroundColor DarkGray "========================================================================="
        $RobocopyWorkspace = $false
    }
    elseif (! (Test-Path "$WorkspacePath\Media\sources\boot.wim")) {
        Write-Warning "$((Get-Date).ToString('OSDy-MM-dd-HHmmss')) OSDCloud WinPE does not exist at $WorkspacePath\Media\sources\boot.wim"
        Write-Warning "$((Get-Date).ToString('OSDy-MM-dd-HHmmss')) You will not be able to update the WinPE volume"
        Write-Host -ForegroundColor DarkGray "========================================================================="
        $RobocopyWorkspace = $false
    }
    ...
    if ($RobocopyWorkspace) {
        ...
        if (! (Test-Path "$PowerShellPath")) {
            Write-Host -ForegroundColor DarkGray "$((Get-Date).ToString('OSDy-MM-dd-HHmmss')) Creating OSDCloud Workspace PowerShell at $WorkspacePath\PowerShell"
    ...
    if ($RobocopyWorkspace) {
        if ($UpdateModules -or $PSUpdate) {
            Write-Host -ForegroundColor DarkGray "$((Get-Date).ToString('OSDy-MM-dd-HHmmss')) Updating OSDCloud Workspace PowerShell Modules and Scripts at $PowerShellPath"
        
            try {
                Save-Module OSD -Path "$PowerShellPath\Offline\Modules" -ErrorAction Stop
            }
            catch {
                Write-Warning "$((Get-Date).ToString('OSDy-MM-dd-HHmmss')) There were some issues updating the OSD PowerShell Module at $PowerShellPath\Offline\Modules"
                Write-Warning "$((Get-Date).ToString('OSDy-MM-dd-HHmmss')) Make sure you have an Internet connection and can access powershellgallery.com"
            }
            ...
            try {
                ...
                Write-Warning "$((Get-Date).ToString('OSDy-MM-dd-HHmmss')) There were some issues updating the WindowsAutoPilotIntune PowerShell Module at $PowerShellPath\Offline\Modules"
                Write-Warning "$((Get-Date).ToString('OSDy-MM-dd-HHmmss')) Make sure you have an Internet connection and can access powershellgallery.com"
            }
            ...
            try {
                ...
                Write-Warning "$((Get-Date).ToString('OSDy-MM-dd-HHmmss')) There were some issues updating the Get-WindowsAutopilotInfo PowerShell Script at $PowerShellPath\Offline\Scripts"
                Write-Warning "$((Get-Date).ToString('OSDy-MM-dd-HHmmss')) Make sure you have an Internet connection and can access powershellgallery.com"
            }
        }
    }
    ...
    if ($WinpeVolumes) {
        if ($IsAdmin) {
            Write-Host -ForegroundColor DarkGray "$((Get-Date).ToString('OSDy-MM-dd-HHmmss')) Setting OSDCloud USB WinPE volume labels to WinPE"
            ...
            Write-Warning "$((Get-Date).ToString('OSDy-MM-dd-HHmmss')) Unable to set OSDCloud USB WinPE volume label"
            Write-Warning "$((Get-Date).ToString('OSDy-MM-dd-HHmmss')) Run this function again elevated with Admin rights"
        }
    }
    #=================================================
    ...
    if ($WinpeVolumes -and $RobocopyWorkspace) {
        foreach ($WinpeVolume in $WinpeVolumes) {
            if (Test-Path -Path "$($WinPEVolume.DriveLetter):\") {
                Write-Host -ForegroundColor DarkGray "$((Get-Date).ToString('OSDy-MM-dd-HHmmss')) ROBOCOPY $WorkspacePath\Media $($WinPEVolume.DriveLetter):\"
    ...
    if ($OSDCloudVolumes -and $RobocopyWorkspace) {
        foreach ($OSDCloudVolume in $OSDCloudVolumes) {
            if (Test-Path "$WorkspacePath\Config") {
                Write-Host -ForegroundColor DarkGray "$((Get-Date).ToString('OSDy-MM-dd-HHmmss')) ROBOCOPY $WorkspacePath\Config $($OSDCloudVolume.DriveLetter):\OSDCloud\Config"
            ...
            if (Test-Path "$WorkspacePath\DriverPacks") {
                Write-Host -ForegroundColor DarkGray "$((Get-Date).ToString('OSDy-MM-dd-HHmmss')) ROBOCOPY $WorkspacePath\DriverPacks $($OSDCloudVolume.DriveLetter):\OSDCloud\DriverPacks"
            ...
            if (Test-Path "$WorkspacePath\OS") {
                Write-Host -ForegroundColor DarkGray "$((Get-Date).ToString('OSDy-MM-dd-HHmmss')) ROBOCOPY $WorkspacePath\OS $($OSDCloudVolume.DriveLetter):\OSDCloud\OS"
            ...
            if (Test-Path "$WorkspacePath\PowerShell") {
                Write-Host -ForegroundColor DarkGray "$((Get-Date).ToString('OSDy-MM-dd-HHmmss')) ROBOCOPY $WorkspacePath\PowerShell $($OSDCloudVolume.DriveLetter):\OSDCloud\PowerShell"
    ...
    if (! ($OSDCloudVolumes)) {
        Write-Warning "$((Get-Date).ToString('OSDy-MM-dd-HHmmss')) Unable to find an OSDCloud USB volume"
        Write-Warning "$((Get-Date).ToString('OSDy-MM-dd-HHmmss')) The USB volume must be labeled OSDCloud and be at least 8GB in size"
        Write-Host -ForegroundColor DarkGray "========================================================================="
        Break
    }

    if (($OSDCloudVolumes | Measure-Object).Count -gt 1) {
        Write-Host -ForegroundColor Yellow "$((Get-Date).ToString('OSDy-MM-dd-HHmmss')) Select a single OSDCloud USB volume in PowerShell GridView and press OK"
        $OSDCloudVolumes = $OSDCloudVolumes | Out-GridView -Title 'Select an OSDCloud USB volume and press OK' -PassThru -OutputMode Single
    }

    if (! ($OSDCloudVolumes)) {
        Write-Warning "$((Get-Date).ToString('OSDy-MM-dd-HHmmss')) You must select one OSDCloud USB volume"
        Write-Host -ForegroundColor DarkGray "========================================================================="
        Break
    }
    #=================================================
    ...
    if (($OSDCloudVolumes | Measure-Object).Count -eq 1) {
        if ($OS -or $OSName -or $OSLicense -or $OSLanguage) {
            ...
            if ($OperatingSystems) {
                ...
                Write-Host -ForegroundColor Yellow "$((Get-Date).ToString('OSDy-MM-dd-HHmmss')) Select one or more Operating Systems to download in PowerShell GridView"
                ...
                foreach ($OperatingSystem in $OperatingSystems) {
                    ...
                    elseif (Test-WebConnection -Uri "$($OperatingSystem.FileUri)") {
                        ...
                        Write-Host -ForegroundColor DarkGray "$((Get-Date).ToString('OSDy-MM-dd-HHmmss')) Downloading OSDCloud Operating System to $OSDownloadChildPath"
                        ...
                        if (Test-Path $SaveWebFile.FullName) {
                            ...
                            Write-Warning "$((Get-Date).ToString('OSDy-MM-dd-HHmmss')) Could not download the Operating System"
                        }
                    }
                    else {
                        Write-Warning "$((Get-Date).ToString('OSDy-MM-dd-HHmmss')) Could not verify an Internet connection for the Operating System"
                    }
                }
            }
            else {
                Write-Warning "$((Get-Date).ToString('OSDy-MM-dd-HHmmss')) Unable to determine a suitable Operating System"
            }
            Write-Host -ForegroundColor DarkGray "========================================================================="
        }
        else {
    ...
    if (($OSDCloudVolumes | Measure-Object).Count -eq 1) {
        if ($DriverPack) {
            Write-Host -ForegroundColor DarkGray "========================================================================="
            Write-Host -ForegroundColor DarkGray "$((Get-Date).ToString('OSDy-MM-dd-HHmmss')) DriverPacks will require up to 2GB each"
    ...
    if ($UpdateModules -or $PSUpdate) {
        if (($OSDCloudVolumes | Measure-Object).Count -eq 1) {
            Write-Host -ForegroundColor DarkGray "$((Get-Date).ToString('OSDy-MM-dd-HHmmss')) Updating OSD and WindowsAutoPilotIntune PowerShell Modules at $PowerShellPath"
    ...
    #   Complete
    #=================================================
    Write-Host -ForegroundColor Cyan "$((Get-Date).ToString('OSDy-MM-dd-HHmmss')) Update-OSDCloudUSB is complete"
    #=================================================
}