function Import-MDTWinPECloudDriver {
    ...
    if ($MDTPSDrive) {
        ...
        if (! (Test-Path 'OSDCloudMDT:\Selection Profiles\OSDCloud WinPE x64')) {
            New-Item -Path 'OSDCloudMDT:\Selection Profiles' -Enable 'True' -Name 'OSDCloud WinPE x64' -Comments 'OSDCloud OSDCloud WinPE x64' -Definition "<SelectionProfile><Include path=`"Out-of-Box Drivers\OSDCloud WinPE x64`" ></Include></SelectionProfile>" -ReadOnly 'False' -Verbose
        }
        #=================================================
        #   Create Directory for CloudDrivers
        #=================================================
        if (! (Test-Path 'OSDCloudMDT:\Out-of-Box Drivers\OSDCloud WinPE x64')) {
            New-Item -Path 'OSDCloudMDT:\Out-of-Box Drivers' -Enable 'True' -Name 'OSDCloud WinPE x64' -Comments 'OSDCloud OSDCloud WinPE x64' -ItemType Folder -Verbose
        }
        #=================================================
        #   DriverHWID
        #=================================================
        ...
        foreach ($CloudDriverName in $CloudDriver) {
            if (! (Test-Path "OSDCloudMDT:\Out-of-Box Drivers\OSDCloud WinPE x64\$CloudDriverName")) {
                New-Item -Path 'OSDCloudMDT:\Out-of-Box Drivers\OSDCloud WinPE x64' -Enable 'True' -Name $CloudDriverName -Comments '' -ItemType Folder -Verbose
            }
            $WinPECloudDriver = Save-WinPECloudDriver -CloudDriver $CloudDriverName
            Import-MDTDriver -Path "OSDCloudMDT:\Out-of-Box Drivers\OSDCloud WinPE x64\$CloudDriverName" -SourcePath $WinPECloudDriver.FullName -Verbose
        }
