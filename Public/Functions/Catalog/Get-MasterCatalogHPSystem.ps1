...
.NOTES
#>
function Get-MasterCatalogHPSystem {
    [CmdletBinding()]
    param (
    ...
    $BuildCatalogFile		= Join-Path $env:TEMP (Join-Path 'OSD' 'MasterCatalogHPSystem.xml')
    $OfflineCatalogFile     = "$($MyInvocation.MyCommand.Module.ModuleBase)\Catalogs\MASTER\MasterCatalogHPSystem.xml"
    ...
    if ($UseCatalog -eq 'Raw') {
        ...
        Get-MasterCatalogHPPlatformList | ForEach-Object{
