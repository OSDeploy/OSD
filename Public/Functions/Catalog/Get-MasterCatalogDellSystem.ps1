...
.NOTES
#>
function Get-MasterCatalogDellSystem {
    [CmdletBinding()]
    param (
    ...
    $BuildCatalogFile		= Join-Path $env:TEMP (Join-Path 'OSD' 'MasterCatalogDellSystem.xml')
    $OfflineCatalogFile		= "$($MyInvocation.MyCommand.Module.ModuleBase)\Catalogs\MASTER\MasterCatalogDellSystem.xml"
