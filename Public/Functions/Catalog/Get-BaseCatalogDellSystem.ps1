...
.NOTES
#>
function Get-BaseCatalogDellSystem {
    [CmdletBinding()]
    param (
    ...
    $BuildCatalogFile		= Join-Path $env:TEMP (Join-Path 'OSD' 'BaseCatalogDellSystem.xml')
    $OfflineCatalogFile		= "$($MyInvocation.MyCommand.Module.ModuleBase)\Catalogs\BASE\BaseCatalogDellSystem.xml"
