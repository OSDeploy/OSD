function New-OSDCloudUSB {
    ...
    if ($PSCmdlet.ParameterSetName -eq 'fromIsoFile') {
        ...
        if ($fromIsoFileGetItem -and $fromIsoFileGetItem.Extension -eq '.iso') {
            ...
            Write-Warning "[$(Get-Date -format s)] Unable to get the properties of $fromIsoFile"
        ...
        Write-Host -ForegroundColor DarkGray "[$(Get-Date -format s)] Mounting OSDCloudISO at $fromIsoFileFullName"
        ...
        if ($MountDiskImageDriveLetter) {
            ...
            Write-Warning "[$(Get-Date -format s)] Unable to mount $MountDiskImage"
            Write-Warning "[$(Get-Date -format s)] Something went very very wrong in this process"
            Break
        }
    }
    ...
    if (-NOT ($WinpeDestinationPath)) {
        Write-Warning "[$(Get-Date -format s)] Unable to find Destination Path at $WinpeDestinationPath"
        Break
    }
    #=================================================
    #	Update WinPE Volume
    ...
    if ((Test-Path -Path "$WinpeSourcePath") -and (Test-Path -Path "$WinpeDestinationPath")) {
        Write-Host -ForegroundColor DarkGray "[$(Get-Date -format s)] Copying $WinpeSourcePath to OSDCloud WinPE partition at $WinpeDestinationPath"
    ...
    #   Complete
    #=================================================
    Write-Host -ForegroundColor Cyan "[$(Get-Date -format s)] New-OSDCloudUSB is complete"
    #=================================================
}
...
function Update-OSDCloudUSB {
    ...
    param (
        ...
        [ValidateSet(
            'Windows 11 25H2','Windows 11 24H2','Windows 11 23H2','Windows 11 22H2','Windows 11 21H2',
    ...
    if ($UsbVolumes) {
        Write-Host -ForegroundColor DarkGray "[$(Get-Date -format s)] USB volumes found"
        Write-Host -ForegroundColor DarkGray "========================================================================="
    }
    else {
        Write-Warning "[$(Get-Date -format s)] Unable to find any USB volumes"
        Write-Warning "[$(Get-Date -format s)] Plug in a USB drive first"
        Write-Host -ForegroundColor DarkGray "========================================================================="
        Break
    }
    #=================================================
    ...
    #=================================================
    if (! $WorkspacePath) {
        Write-Warning "[$(Get-Date -format s)] OSDCloud Workspace is not present on this system"
        Write-Warning "[$(Get-Date -format s)] You will not be able to update the WinPE volume"
        Write-Host -ForegroundColor DarkGray "========================================================================="
        $RobocopyWorkspace = $false
    }
    elseif (! (Test-Path $WorkspacePath)) {
        Write-Warning "[$(Get-Date -format s)] OSDCloud Workspace is not at the path $WorkspacePath"
        Write-Warning "[$(Get-Date -format s)] You will not be able to update the WinPE volume"
        Write-Host -ForegroundColor DarkGray "========================================================================="
        $RobocopyWorkspace = $false
    }
    elseif (! (Test-Path "$WorkspacePath\Media\sources\boot.wim")) {
        Write-Warning "[$(Get-Date -format s)] OSDCloud WinPE does not exist at $WorkspacePath\Media\sources\boot.wim"
        Write-Warning "[$(Get-Date -format s)] You will not be able to update the WinPE volume"
        Write-Host -ForegroundColor DarkGray "========================================================================="
        $RobocopyWorkspace = $false
    }
    ...
    if ($WinpeVolumes) {
        if ($IsAdmin) {
            Write-Host -ForegroundColor DarkGray "[$(Get-Date -format s)] Setting OSDCloud USB WinPE volume labels to WinPE"
            foreach ($volume in $WinpeVolumes) {
                Set-Volume -DriveLetter $volume.DriveLetter -NewFileSystemLabel 'WinPE' -ErrorAction Ignore
            }
        }
        else {
            Write-Warning "[$(Get-Date -format s)] Unable to set OSDCloud USB WinPE volume label"
            Write-Warning "[$(Get-Date -format s)] Run this function again elevated with Admin rights"
        }
    }
    #=================================================
    ...
    if ($WinpeVolumes -and $RobocopyWorkspace) {
        foreach ($volume in $WinpeVolumes) {
            if (Test-Path -Path "$($volume.DriveLetter):\") {
                Write-Host -ForegroundColor DarkGray "[$(Get-Date -format s)] ROBOCOPY $WorkspacePath\Media $($volume.DriveLetter):\"
    ...
    if ($RobocopyWorkspace) {
        if ($UpdateModules -or $PSUpdate) {
            Write-Host -ForegroundColor DarkGray "[$(Get-Date -format s)] Updating OSDCloud Workspace PowerShell Modules and Scripts at $PowerShellPath"
            ...
            try {
                ...
                Write-Warning "[$(Get-Date -format s)] There were some issues updating the Get-WindowsAutopilotInfo PowerShell Script at $PowerShellPath\Offline\Scripts"
                Write-Warning "[$(Get-Date -format s)] Make sure you have an Internet connection and can access powershellgallery.com"
            }
        }
    }
    ...
    if ($OSDCloudVolumes) {
        if ($IsAdmin) {
            Write-Host -ForegroundColor DarkGray "[$(Get-Date -format s)] Setting OSDCloud USB volume labels to OSDCloudUSB"
            ...
            Write-Warning "[$(Get-Date -format s)] Unable to set OSDCloud USB volume label"
            Write-Warning "[$(Get-Date -format s)] Run this function again elevated with Admin rights"
        }
    }
    #=================================================
        ...
        foreach ($volume in $OSDCloudVolumes) {
            if ($IsOfflineReady -or $UpdateModules -or $PSUpdate -or $DriverPack -or $OS -or $OSName -or $OSActivation -or $OSLanguage) {
                if (Test-Path "$WorkspacePath\Config") {
                    Write-Host -ForegroundColor DarkGray "[$(Get-Date -format s)] ROBOCOPY $WorkspacePath\Config $($volume.DriveLetter):\OSDCloud\Config"
    ...
    if ($OSDCloudVolumes) {
        if ($DriverPack -or $OS -or $OSName -or $OSActivation -or $OSLanguage) {
            if (! ($OSDCloudVolumes)) {
                Write-Warning "[$(Get-Date -format s)] Unable to find an OSDCloud USB volume"
                Write-Warning "[$(Get-Date -format s)] The USB volume must be labeled OSDCloud and be at least 8GB in size"
                Write-Host -ForegroundColor DarkGray "========================================================================="
                Break
            }
    ...
    if (($OSDCloudVolumes | Measure-Object).Count -eq 1) {
        if ($OS -or $OSName -or $OSActivation -or $OSLanguage) {
            ...
            if ($OperatingSystems) {
                ...
                Write-Host -ForegroundColor Yellow "[$(Get-Date -format s)] Select one or more Operating Systems to download in PowerShell GridView"
                ...
                foreach ($OperatingSystem in $OperatingSystems) {
                    ...
                    elseif (Test-WebConnection -Uri "$($OperatingSystem.Url)") {
                        ...
                        Write-Host -ForegroundColor DarkGray "[$(Get-Date -format s)] Downloading OSDCloud Operating System to $OSDownloadChildPath"
                        ...
                        if (Test-Path $SaveWebFile.FullName) {
                            ...
                            Write-Warning "[$(Get-Date -format s)] Could not download the Operating System"
                        }
                    }
                    else {
                        Write-Warning "[$(Get-Date -format s)] Could not verify an Internet connection for the Operating System"
                    }
                }
            }
            else {
                Write-Warning "[$(Get-Date -format s)] Unable to determine a suitable Operating System"
            }
            Write-Host -ForegroundColor DarkGray "========================================================================="
        }
        else {
    ...
    if ($OSDCloudVolumes -and $IsOfflineReady) {
        if ($UpdateModules -or $PSUpdate) {
            if (($OSDCloudVolumes | Measure-Object).Count -eq 1) {
                if (Test-Path "$($OSDCloudVolumes.DriveLetter):\OSDCloud") {
                    Write-Host -ForegroundColor DarkGray "[$(Get-Date -format s)] Updating OSD and WindowsAutoPilotIntune PowerShell Modules at $PowerShellPath"
    ...
    Write-Host -ForegroundColor Gray "Update-OSDCloudUSB -OSName 'Windows 11 25H2'"
    ...
    Write-Host -ForegroundColor Gray "Update-OSDCloudUSB -OSName 'Windows 11 25H2' -OSLanguage en-us"
    Write-Host -ForegroundColor Gray "Update-OSDCloudUSB -OSName 'Windows 11 25H2' -OSLanguage de-de -OSActivation Volume"
    Write-Host -ForegroundColor DarkGray "========================================================================="
    ...
    Write-Host -ForegroundColor DarkGray "========================================================================="
    Write-Host -ForegroundColor Cyan "[$(Get-Date -format s)] Update-OSDCloudUSB is complete"
    #=================================================
}
