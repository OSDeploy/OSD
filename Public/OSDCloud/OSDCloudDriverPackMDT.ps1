function Invoke-OSDCloudDriverPackMDT {
    ...
    Write-Host -ForegroundColor DarkGray "[$(Get-Date -format s)] DriverPacks will be downloaded to $OSDiskDrivers"
    ...
    #	Get-MyDriverPack
    #=================================================
    Write-Host -ForegroundColor DarkGray "[$(Get-Date -format s)] Processing function Get-MyDriverPack"
    Write-Host
    ...
    if ($GetMyDriverPack) {     
        ...
        Write-Warning "[$(Get-Date -format s)] There are no DriverPacks for this computer"
        Start-Sleep -Seconds 5
        Continue
    }
    #=================================================
    ...
    if ($GetMyDriverPack) {
        ...
        if ($DeployRootDriverPack) {
            Write-Host -ForegroundColor Cyan "[$(Get-Date -format s)] Copying existing DriverPack from the MDT DeploymentShare"
    ...
    if ($GetMyDriverPack) {
        ...
        if (-Not ($ReadyDriverPack)) {
            if ((-NOT (Test-Path "$env:SystemRoot\System32\curl.exe")) -and (-NOT (Test-Path "$OSDISK\Windows\System32\curl.exe"))) {
                Write-Warning "[$(Get-Date -format s)] Curl is required for this to function"
                Start-Sleep -Seconds 5
                Continue
            }
        ...
        $ReadyDriverPack = Get-ChildItem -Path $OSDiskDrivers -File -Force -ErrorAction Ignore | Where-Object {$_.Name -match $GetMyDriverPackBaseName} | Where-Object {$_.Extension -in ('.cab','.zip','.exe')}
        if ($ReadyDriverPack) {
            Write-Host -ForegroundColor Cyan "[$(Get-Date -format s)] DriverPack has been copied to $OSDiskDrivers"
            ...
            if (Test-Path $OSDCloudDriverPackPPKG) {
                Write-Host
                Write-Host -ForegroundColor Cyan "[$(Get-Date -format s)] Applying first boot Specialize Provisioning Package"
            ...
            Write-Warning "[$(Get-Date -format s)] Could not download the DriverPack"
        }
        Stop-Transcript
        Start-Sleep -Seconds 5
    }
