function Get-OSDCloudWorkspace {
    ...
    if (Test-Path "$env:ProgramData\OSDCloud\workspace.json") {
        ...
        Write-Warning "[$(Get-Date -format s)] Unable to locate $env:ProgramData\OSDCloud\workspace.json"
    }
    #endregion
}
function New-OSDCloudWorkspace {
    ...
    if ($PSCmdlet.ParameterSetName -eq 'fromTemplate') {
        ...
        if (Test-Path "$(Get-OSDCloudTemplate)\Autopilot") {
            Write-Warning "[$(Get-Date -format s)] Move all your Autopilot Profiles to $(Get-OSDCloudTemplate)\Config\AutopilotJSON"
            Write-Warning "[$(Get-Date -format s)] You will be unable to create or update an OSDCloud Workspace until $(Get-OSDCloudTemplate)\Autopilot is manually removed"
            Break
        }
        if (Test-Path "$WorkspacePath\Autopilot") {
            Write-Warning "[$(Get-Date -format s)] Move all your Autopilot Profiles to $WorkspacePath\Config\AutopilotJSON"
            Write-Warning "[$(Get-Date -format s)] You will be unable to create or update an OSDCloud Workspace until $WorkspacePath\Autopilot is manually removed"
            Break
        }
    }
    #endregion
    ...
    if (Test-Path $WorkspacePath) {
        Write-Warning "[$(Get-Date -format s)] OSDCloud Workspace already exists at $WorkspacePath"
        Write-Warning "[$(Get-Date -format s)] Content will be merged and overwritten"
        Write-Warning "[$(Get-Date -format s)] Press Ctrl+C to cancel in the next 5 seconds"
        Start-Sleep -Seconds 5
    }
    else {
        Write-Host -ForegroundColor DarkGray "[$(Get-Date -format s)] New-Item $WorkspacePath"
        try {
            $null = New-Item -Path $WorkspacePath -ItemType Directory -Force -ErrorAction Stop
        }
        catch {
            Write-Warning "[$(Get-Date -format s)] Unable to create an OSDCloud Workspace at $WorkspacePath"
            Break
        }
    }
    #endregion
    ...
    Write-Host -ForegroundColor DarkGray "[$(Get-Date -format s)] Creating OSDCloud Workspace Logs at $WorkspaceLogs"
    ...
    if ($PSCmdlet.ParameterSetName -eq 'fromTemplate') {
        ...
        #	Mirror Media
        #=================================================
        Write-Host -ForegroundColor DarkGray "[$(Get-Date -format s)] Mirroring OSDCloud Template Media using Robocopy"
        Write-Host -ForegroundColor DarkGray "[$(Get-Date -format s)] Mirroring will replace any previous WinPE with a new Template WinPE"
        Write-Host -ForegroundColor DarkGray "[$(Get-Date -format s)] Directories named OSDCloud are updated"
        Write-Host -ForegroundColor DarkGray "[$(Get-Date -format s)] Source: $OSDCloudTemplate\Media"
        Write-Host -ForegroundColor DarkGray "[$(Get-Date -format s)] Destination: $WorkspacePath\Media"
    ...
    elseif ($PSCmdlet.ParameterSetName -eq 'fromUsbDrive') {
        ...
        if ($WinpeVolumes) {
            foreach ($WinpeVolume in $WinpeVolumes) {
                if (Test-Path -Path "$($WinPEVolume.DriveLetter):\") {
                    Write-Host -ForegroundColor DarkGray "[$(Get-Date -format s)] Copying OSDCloud WinPE volume at $($WinPEVolume.DriveLetter):\ to $WorkspacePath\Media"
            ...
            foreach ($OSDCloudVolume in $OSDCloudVolumes) {
                if (! $Public) {
                    if (Test-Path "$($OSDCloudVolume.DriveLetter):\OSDCloud\Config") {
                        Write-Host -ForegroundColor DarkGray "[$(Get-Date -format s)] Copying $($OSDCloudVolume.DriveLetter):\OSDCloud\Config to OSDCloud Workspace $WorkspacePath\Config"
        ...
        if ((Test-Path -Path "$WinpeSourcePath") -and (Test-Path -Path "$WinpeDestinationPath")) {
            Write-Host -ForegroundColor DarkGray "[$(Get-Date -format s)] Copying $WinpeSourcePath to $WinpeDestinationPath"
    ...
    if ($MountDiskImage) {
        Start-Sleep -Seconds 3
        Write-Host -ForegroundColor DarkGray "[$(Get-Date -format s)] Dismounting $($MountDiskImage.ImagePath)"
    ...
    Write-Host -ForegroundColor DarkGray "========================================================================="
    Write-Host -ForegroundColor Cyan "[$(Get-Date -format s)] New-OSDCloudWorkspace created at $WorkspacePath"
