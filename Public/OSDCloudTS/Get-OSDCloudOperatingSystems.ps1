function Get-OSDCloudOperatingSystems {
    ...
    $FullResults = Get-Content -Path "$(Get-OSDCatalogsPath)\osd-catalogs\CloudOperatingSystems.json" | ConvertFrom-Json
    if ($OSArch -eq 'x64'){
        $Results = $FullResults | Where-Object {$_.Architecture -eq "x64"}
    }
    elseif ($OSArch -eq "arm64"){
        $Results = Get-Content -Path "$(Get-OSDCatalogsPath)\osd-catalogs\CloudOperatingSystemsARM64.json" | ConvertFrom-Json
    }
    $Results
}