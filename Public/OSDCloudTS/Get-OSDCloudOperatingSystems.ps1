function Get-OSDCloudOperatingSystems {
    ...
    $FullResults = Get-Content -Path "$(Get-OSDCatsPath)\osd-module\CloudOperatingSystems.json" | ConvertFrom-Json
    if ($OSArch -eq 'x64'){
        $Results = $FullResults | Where-Object {$_.Architecture -eq "x64"}
    }
    elseif ($OSArch -eq "arm64"){
        $Results = Get-Content -Path "$(Get-OSDCatsPath)\osd-module\CloudOperatingSystemsARM64.json" | ConvertFrom-Json
    }
    $Results
}